...
class TestSetupPlanningSession:
    ...
    def test_new_session_with_existing_spec(self, run_script, tmp_path):
        ...
        # Session isolation: planning_dir is sessions/<prefix> under spec parent
        assert output["planning_dir"].startswith(str(tmp_path))
        assert "/sessions/" in output["planning_dir"]
...
"""
    ...
    def test_creates_session_config(self, run_script, tmp_path, plugin_root):
        ...
        # Config file should exist in session-isolated planning dir
        output = json.loads(result.stdout)
        planning_dir = Path(output["planning_dir"])
        ...
        assert config["planning_dir"] == str(planning_dir)

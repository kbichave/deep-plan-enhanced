...
The planning directory is a session-isolated subdirectory under the spec file's parent
(sessions/<session-prefix>/) to prevent concurrent sessions from overwriting each other.
...
2. Resolves session-isolated planning directory (sessions/<8-char-prefix>/ under spec parent)
3. Creates `deep_plan_config.json` in the planning directory with `plugin_root`, `planning_dir`, `initial_file`, and `session_id`
4. Detects whether this is a new or resume session
5. Writes task files directly to `~/.claude/tasks/<task_list_id>/`
6. If `sections/index.md` exists, also writes section tasks (positions 22+)

**Session isolation:** Each session writes to its own subdirectory. Two concurrent
`/deep-plan` sessions on the same spec file will NOT overwrite each other's plans.
...
**IMPORTANT — Progress Tracking:** After completing each step, update `<planning_dir>/progress.md`:
1. Check off the completed step: change `- [ ]` to `- [x]`
2. If errors occurred, add a row to the Errors Encountered table
3. Add any notable decisions or findings to the Notes section

This keeps the progress file in sync for recovery after `/clear` or context compaction.
...
Read `{plugin_root}/references/context-check.md` for handling the output.
...
Read `{plugin_root}/references/external-review.md` for the full protocol.
...
Read `{plugin_root}/references/section-splitting.md` for the batch execution loop.

...
class TestCaptureSessionIdHook:
    ...
    def test_outputs_session_id_as_additional_context(self, hook_module, capsys):
        ...
        assert "DEEP_PLUGIN_ROOT=" in context
    ...
    def test_skips_output_when_deep_session_id_matches(self, hook_module, capsys):
        ...
        assert result == 0
        captured = capsys.readouterr()
        # Session ID matches so not re-emitted, but plugin_root is always emitted
        output = json.loads(captured.out)
        context = output["hookSpecificOutput"]["additionalContext"]
    ...
    def test_outputs_when_deep_session_id_differs(self, hook_module, capsys):
        ...
        output = json.loads(captured.out)
        context = output["hookSpecificOutput"]["additionalContext"]
        assert "DEEP_SESSION_ID=new-session-456" in context
    ...
    def test_outputs_when_deep_session_id_not_set(self, hook_module, capsys):
        ...
        output = json.loads(captured.out)
        context = output["hookSpecificOutput"]["additionalContext"]
        assert "DEEP_SESSION_ID=test-session-789" in context
        assert "DEEP_PLUGIN_ROOT=" in context

    def test_always_includes_plugin_root(self, hook_module, capsys):
        """Should always include DEEP_PLUGIN_ROOT derived from script location."""
        payload = {"session_id": "test-session-123"}
    ...
    def test_plugin_root_when_session_id_matches(self, hook_module, capsys):

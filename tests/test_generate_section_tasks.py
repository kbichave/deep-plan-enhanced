...
"""
    ...
    def test_output_includes_task_list_context(self, run_script, tmp_path, sample_index_content):
        ...
        # Clear session env vars so test gets "none" source
        orig_session = os.environ.pop("DEEP_SESSION_ID", None)
        orig_task_list = os.environ.pop("CLAUDE_CODE_TASK_LIST_ID", None)
        try:
            result = run_script(planning_dir)
        finally:
            if orig_session is not None:
                os.environ["DEEP_SESSION_ID"] = orig_session
            if orig_task_list is not None:
                os.environ["CLAUDE_CODE_TASK_LIST_ID"] = orig_task_list
